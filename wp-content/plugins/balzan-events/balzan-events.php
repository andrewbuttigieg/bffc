<?php
	/*
	Plugin Name: BalzanFC Coming Events
	Description: Coming Events Builder
	...
	*/
	...
	function custom_post_section() {
		$labels = array(
			'name' => _x('Event', 'post type general name'),
			'singluar_name' => _x('Event', 'post type singular name'),
			...
			'parent_item_colon' => '',
			'menu_name' => 'Coming Events'
		);

		$args = array(
			'labels' => $labels,
			'description' => 'Stores the home page Events',
			'public' => true,
			'menu_position' => 11,
	...
	add_action('init', 'custom_post_section');
	...
	add_action('load-post.php', 'section_post_meta_box_setup');
	add_action('load-post-new.php', 'section_post_meta_box_setup');

	// Meta box setup function
	function section_post_meta_box_setup() {
		// Add meta boxes on the 'add_meta_boxes' hook
		add_action('add_meta_boxes', 'section_add_post_meta_box');
		// Save post meta on the 'save_post' hook
		add_action('save_post', 'section_save_post_meta', 10, 2);
	}

	// Create one or more meta boxes to be displayed on the post editor screen
	function section_add_post_meta_box() {
		add_meta_box(
			'section-post-data',	// Unique ID
			'Section Details',	// Title
			'show_section_post_meta_box',	// Callback function
			'section',		// post type
			'normal',		// context
			'default'		// priority
		);
	...
	function show_section_post_meta_box() {
	...
	function section_post_get_field($field, $meta) {
		switch ($field['type']) {
			// text
			case 'text':
	...
	function get_Event() {
		...
		$output .= '<tr>';
		$output .= '<th><label>Select Event</label><input id="special_offers_cnt" name="special_offers_cnt" type="hidden" value="'
		...
		if($special_offers_cnt > 0) {
			for($i=0; $i < $special_offers_cnt; $i++) {
				...
				$output .= '</td>';
				$output .= '<td>';
				$output .= '<input id="upload_link" type="text" size="18" name="the_link_'.$i.'" value="' . $special_offers[$i]->link . '" />';
				$output .= '</td>';
		...
		$output .= '<label>Event title</label><label for="upload_title">';
		$output .= '</td><td>';
		$output .= '<input type="text" size="18" name="the_text_'.$special_offers_cnt.'" value="" /><br/><label>Event text</label></td>';
		$output .= '</td><td>';
		$output .= '<input type="text" size="18" name="the_link_'.$special_offers_cnt.'" value="" /><br/><label>Event link</label></td>';
	...
	function section_save_post_meta($post_id) {
	...
	if (is_admin()) {
		...
		wp_enqueue_script('plugin_js', home_url().'/wp-content/plugins/balzan-events/js/plugin_js.js');
	...
	add_action('wp_enqueue_scripts', 'todo_enqeue_scripts');
?>
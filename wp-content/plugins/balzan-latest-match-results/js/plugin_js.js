jQuery(document).ready(function($) {
	...
	$('.add_item').live('click', function() {
		...
		if($('input[name=the_title_' + cnt + ']').val() == '' || $('input[name=the_text_' + cnt + ']').val() == '') {
			alert('To add this event, you will need to assign a title and text');
		} else {			
			var temp_cnt = cnt + 1;
			...
			clone.find('input[name=the_text_' + cnt + ']').val('');
			clone.find('input[name=the_text_' + cnt + ']').attr('name', 'the_text_' + temp_cnt);

			clone.find('input[name=the_title_' + cnt + ']').val('');
			clone.find('input[name=the_title_' + cnt + ']').attr('name', 'the_title_' + temp_cnt);

			clone.find('input[name=the_result_' + cnt + ']').val('');
			clone.find('input[name=the_result_' + cnt + ']').attr('name', 'the_result_' + temp_cnt);
	...
	$('.remove_item').live('click', function() {
		...
		if(confirm_remove) {
			...
			for(i=temp_cnt;i<tot_cnt;i++) {
				...
				temp_row.find('input[name=the_title_' + (row_cnt+1) + ']').attr('name', 'the_title_' + row_cnt);
